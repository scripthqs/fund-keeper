...
export function analyzeFundEnhanced(fund, todayChange, totalReturn, config, peakReturnRate) {
  ...
  if (config.enableStopLoss && totalReturn <= config.stopLossLine) {
    const sellAmount = fund.currentMarketValue * config.stopLossRatio / 100
  ...
  if (config.useTrailingStop) {
    ...
    if (trailing && trailing.triggered) {
      const sellAmount = fund.currentMarketValue * config.stopProfitRatio / 100
...
export function getMultiTierAddBuy(fund, config) {
  ...
  for (const tier of sortedTiers) {
    if (fund.currentReturnRate <= tier.line) {
      return { tierIdx: config.addTiers.indexOf(tier) + 1, line: tier.line, ratio: tier.ratio, buyAmount: fund.initialPrincipal * tier.ratio / 100 }
    }
  }
  return null
}
...
export function getSingleAddBuy(fund, config) {
  ...
  return { minBuy: fund.initialPrincipal * 5 / 100, maxBuy: fund.initialPrincipal * 10 / 100 }
}

/** 安全垫 */
export function calcSafetyCushion(fund, todayChange) {
  const denom = 1 + todayChange / 100
  ...
  const todayProfit = fund.currentMarketValue - yesterdayValue
  const safetyCushion = fund.currentMarketValue > 0 ? (todayProfit / fund.currentMarketValue) * 100 : 0
...
export function calcRecoveryNeeded(totalReturn) {
  ...
  const lossRate = Math.abs(totalReturn) / 100
  if (lossRate >= 1) return 9999  // 跌幅超过100%（理论上不可能），返回极大值
...
export function evaluateWarning(fund, todayChange, totalReturn, config, snapshots) {
  ...
  if (totalReturn < 0) {
    ...
    if (prev && prev.recoveryNeeded != null) {
      const diff = prev.recoveryNeeded - recoveryNeeded
...
export function calcStressTest(fund, dropPercent) {
  ...
  const simMarketValue = fund.currentMarketValue * (1 + drop / 100)
  const simTotalValue = simMarketValue + fund.totalSellAmount
  const simReturnRate = fund.totalBuyAmount > 0 ? ((simTotalValue - fund.totalBuyAmount) / fund.totalBuyAmount) * 100 : 0
  const simLoss = fund.currentMarketValue - simMarketValue
  let recoveryText = ''
  if (simReturnRate < 0) { const lr = Math.abs(simReturnRate) / 100; if (lr >= 1) recoveryText = '模拟后亏损超过100%，已全部亏完'; else recoveryText = `模拟后回本所需涨幅：${((lr / (1 - lr)) * 100).toFixed(1)}%` } else recoveryText = '模拟后仍有浮盈，无需回本'
...
export function calcHealthScore(fund, config, allFunds) {
  ...
  else if (ret > 0) { const b = (ret / config.stopProfitLine) * 15; score += b; details.push({ label: '正收益', score: Math.round(b), color: '#22c55e', text: '持仓盈利中，趋势良好' }) }
  ...
  const totalMarket = allFunds.reduce((s, f) => s + f.currentMarketValue, 0)
  const concentration = totalMarket > 0 ? (fund.currentMarketValue / totalMarket) * 100 : 0
  if (concentration > config.maxPosition) { const p = Math.min(20, Math.round((concentration - config.maxPosition) / 5)); score -= p; details.push({ label: '仓位过重', score: -p, color: '#ef4444', text: `占${concentration.toFixed(1)}%，超过${config.maxPosition}%上限` }) }
